...
export default function NeurIPS2025Page() {
  ...
  const [graphState, setGraphState] = useState<NeurIPSGraphState>({ nodes: [] });
  ...
  const loadData = useCallback(async (k: number = 15) => {
    ...
    try {
      ...
      const nodes: GraphNode[] = paperList.map((paper, idx) => {
        ...
        return {
          id: paper.paper_id,
          label: paper.name.length > 40 ? `${paper.name.substring(0, 37)}...` : paper.name,
          title: paper.name,
          stableKey: neuripsStableKey(paper.paper_id),
          type: 'neurips_paper',
          cluster: clusterId,
          abstract: paper.abstract,
          authors: paper['speakers/authors']
            ? paper['speakers/authors'].split(',').map((author) => author.trim())
            : [],
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      const result = await executeNeurIPSSearchAndRank(searchQuery.trim(), 'users/profile.json', 10, numClusters);
  ...
  useEffect(() => {
    ...
    recordRecentPaper({
      ...
      venue: 'NeurIPS 2025',
      route: `/neurips2025?paper=${encodeURIComponent(selectedPaper.paper_id)}`,
    });
  }, [selectedPaper]);
    ...
    return (
      <div className="workspace-loading-state workspace-stage">
        <div className="workspace-empty-card workspace-floating-panel">
          <div className="workspace-kicker">NeurIPS Workspace</div>
          <h2 className="workspace-title" style={{ fontSize: '30px' }}>Loading paper map</h2>
          <p>Fetching NeurIPS 2025 papers, clusters, and similarity links.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="workspace-stage">
        ...
        {showControls ? (
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px' }}>
              <div>
                <div className="workspace-kicker">NeurIPS 2025</div>
      ...
      {viewMode === 'graph' && (
        <SidePanel
          selectedNode={selectedNode ? { ...selectedNode, label: selectedPaper?.name || selectedNode.label } : null}
